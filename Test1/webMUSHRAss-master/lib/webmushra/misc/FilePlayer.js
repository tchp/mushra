...
function FilePlayer(_audioContext, _bufferSize, _stimuli, _errorHandler) {
...
FilePlayer.prototype.renderElement = function(_parent, _i) {
  var buttonPlay = $('<button data-role="button" id="playCondition'+_i+'" name="'+_i+'">Play</button>');
    buttonPlay.bind( "click", (function(event, ui) {
      this.genericAudioControl.play(parseInt(event.target.name));
    }).bind(this));
    
    var buttonPause = $('<button data-role="button" id="pause">Pause</button>');
