...
function BS1116Page(_reference, _condition, _pageManager, _pageTemplateRenderer, _audioContext, _bufferSize, _audioFileLoader, _session, _pageConfig, _errorHandler) {
...
/*jshint multistr: true */
  ...
  var tdStop = $(" \
    <td class='stopButton' id='tdStopButton'> \
      <button data-role='button' data-inline='true' id='buttonStop' class='center' onclick='"+ this.pageManager.getPageVariableName(this) + ".mushraAudioControl.stop();'>Stop</button> \
    </td> \
  ");
  ...
  var trNames = $("<tr><td>A (Reference)</td><td></td><td>B</td><td>C</td></tr>");
  tableBS1116.append(trNames);
  
  var trPlays = $("<tr></tr>");

  var buttonPlayReference = $("<td id='tdPlayButton' ><button data-theme='a' id='buttonReference' data-role='button' class='audioControlElement' onclick='" + this.pageManager.getPageVariableName(this) + ".btnCallbackReference()' >Play</button></td>");
  ...
  var buttonPlayB = $("<td><button data-theme='a' id='buttonConditions0' data-role='button' class='audioControlElement' onclick='" + this.pageManager.getPageVariableName(this) + ".btnCallbackB()' >Play</button></td>");
  trPlays.append(buttonPlayB);
  var buttonPlayC = $("<td><button data-theme='a' id='buttonConditions1' data-role='button' class='audioControlElement' onclick='" + this.pageManager.getPageVariableName(this) + ".btnCallbackC()' >Play</button></td>");
...
BS1116Page.prototype.renderCanvas = function(_parentId) {
  ...
  canvasContext.fillText("Imperceptible    5", xTextScoreRanges, YfirstLine);
  canvasContext.fillText("Perceptible, but not annoying    4", xTextScoreRanges, prevScalesHeight * 0.25 + YfirstLine);
  canvasContext.fillText("Slightly annoying    3", xTextScoreRanges, prevScalesHeight * 0.5 + YfirstLine);
  canvasContext.fillText("Annoying    2", xTextScoreRanges, prevScalesHeight * 0.75 + YfirstLine);
  canvasContext.fillText("Very annoying    1", xTextScoreRanges, prevScalesHeight + YfirstLine);
...
BS1116Page.prototype.btnCallbackReference = function() {
  ...
  if (label == "Pause") {
    this.mushraAudioControl.pause();
    $("#buttonReference").text("Play");
  } else if (label == "Play") {
    $(".audioControlElement").text("Play");
    this.mushraAudioControl.playReference();
    $("#buttonReference").text("Pause");
...
BS1116Page.prototype.btnCallbackB = function() {
  ...
  if (label == "Pause") {
    this.mushraAudioControl.pause();
    $("#buttonConditions0").text("Play");
  } else if (label == "Play") {
    $(".audioControlElement").text("Play");
    this.mushraAudioControl.playCondition(0);
    $("#buttonConditions0").text("Pause");
...
BS1116Page.prototype.btnCallbackC = function() {
  ...
  if (label == "Pause") {
    this.mushraAudioControl.pause();
    $("#buttonConditions1").text("Play");
  } else if (label == "Play") {
    $(".audioControlElement").text("Play");
    this.mushraAudioControl.playCondition(1);
    $("#buttonConditions1").text("Pause");
