...
function MushraPage(_pageManager, _audioContext, _bufferSize, _audioFileLoader, _session, _pageConfig, _mushraValidator, _errorHandler) {
	this.isMushra = true; 
...
MushraPage.prototype.render = function (_parent) {
  ...
  var tdLoop1 = $(" \
    <td class='stopButton'> \
      <button data-role='button' data-inline='true' id='buttonStop' onclick='"+ this.pageManager.getPageVariableName(this) + ".mushraAudioControl.stop();'>Stop</button> \
    </td> \
  ");
  ...
  var tdConditionNamesReference = $("<td>Reference</td>");
  ...
  var buttonPlayReference = $("<button data-theme='a' id='buttonReference' data-role='button' class='audioControlElement' onclick='" + this.pageManager.getPageVariableName(this) + ".btnCallbackReference()' style='margin : 0 auto;'>Play</button>");
  ...
  for (i = 0; i < conditions.length; ++i) {
    td = $("<td></td>"); 
    var buttonPlay = $("<button data-role='button' class='center audioControlElement' onclick='" + this.pageManager.getPageVariableName(this) + ".btnCallbackCondition(" + i + ");'>Play</button>");
...
MushraPage.prototype.btnCallbackCondition = function(_index) {
  ...
  if (label == "Pause") {
    this.mushraAudioControl.pause();
    $("#buttonConditions" + _index).text("Play");
  } else if (label == "Play") {
    $(".audioControlElement").text("Play");
    this.mushraAudioControl.playCondition(_index);
    $("#buttonConditions" + _index).text("Pause");
  }
};
...
MushraPage.prototype.renderCanvas = function(_parentId) {
  ...
  canvasContext.fillText("Excellent",xLetters , prevScalesHeight * 0.1 + YfirstLine);
  canvasContext.fillText("Good", xLetters, prevScalesHeight * 0.3 + YfirstLine);
  canvasContext.fillText("Fair", xLetters, prevScalesHeight * 0.5 + YfirstLine);
  canvasContext.fillText("Poor", xLetters, prevScalesHeight * 0.7 + YfirstLine);
  canvasContext.fillText("Bad", xLetters, prevScalesHeight * 0.9 + YfirstLine);
