...
for (var i = 0; i < _pages.length; ++i) {
  if (Array.isArray(_pages[i])) {
      ...
      if (pageConfig.type == "generic") {
        ...
        var volumePage = new VolumePage(_pageManager, audioContext, audioFileLoader, pageConfig);
        _pageManager.addPage(volumePage);
      } else if (pageConfig.type == "mushra") {
        var mushraPage = new MushraPage(_pageManager, audioContext, config.bufferSize, audioFileLoader, session, pageConfig, mushraValidator, errorHandler);
        _pageManager.addPage(mushraPage);
      } else if ( pageConfig.type == "spatial"){
      _pageManager.addPage(new SpatialPage(_pageManager, pageConfig, session, audioContext, config.bufferSize, audioFileLoader, errorHandler));		      
      } else if (pageConfig.type == "mushra_results") {
        var mushraResultsPage = new MushraResultsPage(_pageManager, pageConfig, audioContext, config.bufferSize, errorHandler);
        _pageManager.addPage(mushraResultsPage);
      	...
      	pcPageManager.createPages(_pageManager, pageTemplateRenderer, pageConfig, audioContext, config.bufferSize, audioFileLoader, session, errorHandler);
      	pcPageManager = null;
      } else if (pageConfig.type == "bs1116") {
      	var bs1116PageManager = new BS1116PageManager();
      	bs1116PageManager.createPages(_pageManager, pageTemplateRenderer, pageConfig, audioContext, config.bufferSize, audioFileLoader, session, errorHandler);
        ...
        likertSingleStimulusPageManager.createPages(_pageManager, pageTemplateRenderer, pageConfig, audioContext, config.bufferSize, audioFileLoader, session, errorHandler);
        likertSingleStimulusPageManager = null;
      } else if (pageConfig.type == "robert") {
        var robertPageManager = new RobertPageManager();
        robertPageManager.createPages(_pageManager, pageTemplateRenderer, pageConfig, audioContext, config.bufferSize, audioFileLoader, session, errorHandler);
        robertPageManager = null;            
      } else if (pageConfig.type == "likert_multi_stimulus") {
        var likertMultiStimulusPage = new LikertMultiStimulusPage(pageManager, pageTemplateRenderer, pageConfig, audioContext, config.bufferSize, audioFileLoader, session, errorHandler);
        _pageManager.addPage(likertMultiStimulusPage);
      }else if (pageConfig.type == "finish") {
        var finishPage = new FinishPage(_pageManager, session, dataSender, pageConfig);
...
YAML.load(configFile, (function(result) {
    ...
    pageManager = new PageManager("pageManager", "page_content");
    ...
    pageTemplateRenderer = new PageTemplateRenderer(pageManager, config.showButtonPreviousPage);
